...
switch(HOST,
       ...
       'SANDBOX_DOCKER' = {
         ...
         fg_codes_info_table <-
           "finngen-production-library.medical_codes.fg_codes_info_v5"
         code_prevalence_table <- 
           # "fg-production-sandbox-6.sandbox.code_prevalence_stratified_r12_v1"
...
build_plot_values <- function(df_all, values){
  df_all <- df_all |> 
    mutate(chapter = case_when(
      ...
      str_starts(vocabulary_id, "NCSP") & 
        str_detect(FG_CODE1, "^SH(A|B|C)|^SJ(B|C|D|E|F|X)|^SP(A|B|C|D|E|F|G)|^SXC|^OIK") ~ "NCSP_teeth_5_other",
